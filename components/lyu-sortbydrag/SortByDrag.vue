		...
		methods: {
			renderList() {
				this.list.forEach((item, index)=>{
					...
					this.$set(item, 'x', index%4 * this.scaleX)
					this.$set(item, 'y',  Math.floor(index/4) * this.scaleY)
				})
			},
			moveStop(stop) {
				if(this.isMove)
				this.$nextTick(() => {
					...
					temp.forEach((item, index)=>{
						item.x = index%4 * this.scaleX;
						item.y= Math.floor(index/4) * this.scaleY;
					})
					this.list = [];
					this.$nextTick(()=>{
						this.list = temp;
			...
			moving(moveIndexX, moveIndexY,i) {
				const start = i;
				let end = moveIndexY * 4 + moveIndexX + start;
					...
					this.list.forEach((item, index)=>{
						...
						if(start > end) {
							...
							if(index >= end && index < start) {
								item.index = index + 1;
								this.$set(item, 'x', (index + 1)%4 * this.scaleX+1)
								this.$set(item, 'y',  Math.floor((index + 1)/4) * this.scaleY+1)
							}
						}
						if(start < end) {
							// 后移动
